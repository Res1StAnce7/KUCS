...
template <typename T>
void LinkedList<T>::listConcat(ListInterface<T>& destination, ListInterface<T>& copyValues)
{
	for(int i=1; i <=copyValues.length(); i++)
    {   
		destination.insert(destination.length()+1, copyValues.getIndex(i));
    }
}
